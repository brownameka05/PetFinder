function closemyfunction(infoPet) {
  infoPet.parentElement.parentElement.parentElement.removeChild(
    infoPet.parentElement.parentElement
  );
}
...
function showMoreInfo(petID) {
  getPet(petID)
    ...
    <div class = "learnMore" id = ${newPet.id}>
      ...
      <div id="description">
        <p>${
          !jQuery.isEmptyObject(newPet.description) ? newPet.description : ''
        }</p>
      </div>
      <div id = "shelterInfo">
        <h5>Shelter Info</h5>
    ...
    `;
      if (!jQuery.isEmptyObject(newPet.shelterInfo.phone)) {
        petLiteral += `     <p>Phone: ${newPet.shelterInfo.phone['$t']}</p>`;
      }
